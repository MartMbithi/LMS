...
if (isset($_POST['enroll_student'])) {
    ...
    if (!$error) {
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            $stmt->execute();
            if ($stmt) {
                $success = "Added" && header("refresh:1; url=unit_enrollments.php");
            } else {
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $adn = "DELETE FROM lms_enrollments WHERE en_id= '$id' ";
    $stmt = $mysqli->prepare($adn);
    $stmt->execute();
    $stmt->close();
    if ($stmt) {
        $success = "Deleted" && header("refresh:1; url=unit_enrollments.php");
    } else {
        $info = "Please Try Again Or Try Later";
    }
}
