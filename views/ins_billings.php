...
if (isset($_POST['pay_for_reading_material'])) {
    ...
    if (!$error) {
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            $stmt->execute();
            if ($stmt) {
                $success = "Added" && header("refresh:1; url=ins_manage_billings.php");
            } else {
                $info = "Please Try Again Or Try Later";
            }
        }
                    ...
                    <div class="container-fluid">
                        <div class="container">
                            <div class="text-right text-dark">
                                <a class="btn btn-warning" href="ins_manage_billings.php">Manage Study Materials Payments</a>
                            </div>
                        </div>
                        <hr>
                        <div class="row">
                                ...
                                <div class="card-body">
                                    <table id="dash-1" class="table table-striped table-bordered display no-wrap" style="width:100%">
                                        ...
                                        <tbody>
                                            <?php
                                            $id = $_SESSION['i_id'];
                                            $ret = "SELECT  *  FROM  lms_study_material WHERE i_id ='$i_id' ";
                                            $stmt = $mysqli->prepare($ret);
                                            $stmt->execute(); //ok
                                            $res = $stmt->get_result();
